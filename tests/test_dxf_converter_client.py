...
def _base_response(payload: dict) -> dict:
    """Формат corp BaseResponse, как у DXF Converter."""
    return {
        "request_id": "req-1",
...
@pytest.mark.asyncio
async def test_get_preview_png_accepts_flat_response():
    """Обратная совместимость: flat JSON без обёртки data."""
    convert_resp = _mock_response(
        json_data={
            "job_id": "flat123",
            "files": {"png": "flat.png"},
...
async def test_conversion_failed_on_422():
    ...
    err_resp = _mock_response(
        status_code=422,
        json_data={"error": {"code": "HTTP_422", "message": "Invalid DXF structure"}},
    )
    ...
    assert "Invalid DXF" in str(exc_info.value.message)
